...
int main() {
    ...
    // ถ้าเป็น NULL แปลว่า ไม่มีสิทธิ์อ่านไฟล์ หรือไฟล์ไม่มีอยู่จริง
    if ( fp == NULL ) {
    ...
    // หาตำแหน่งของเครื่องหมายขึ้นบรรทัดใหม่ (\n) แล้วแทนที่ด้วย NULL (\n) เพื่อระบุว่าจบข้อความนี้แล้ว
    ...
    for( int i = 0 ; i < len ; i++ ) {
        ...
        // ทำการเลื่อนตัวอักษร 6 ตัวอักษร (ASCII Code)
